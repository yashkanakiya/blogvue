    ...
    <div class="flex flex-col  py-5 bg-gray-400 space-y-3 md:mx-20" >
      ...
      <div>
        ...
        <div class="flex flex-col items-center py-5  bg-green-500 space-y-3 p-10 md:mx-20">
            ...
            <div v-for="(post,index) in posts" :key="index" class="bg-gray-800 text-white p-10 text-center space-y-5">
...
export default {  
 computed : {
   ...
   ...mapState(useStore,['posts'])
 },
created() {
  this.fetchPosts()
},
methods: {
  ...mapActions(useStore,['add', 'rm', 'editPost', 'up', 'fetchPosts']),
  actionBlog() {
    this.add(this.blog),
    this.rm(this.post),
